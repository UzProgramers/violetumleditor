...
public abstract class SegmentedLineEdge extends ShapeEdge {
	...
	public void draw(Graphics2D g2) {
		...
		drawString(g2, (Point2D) points.get(points.size() / 2 - 1), (Point2D) points.get(points.size() / 2), null,
				middleLabel, true);
		drawString(g2, (Point2D) points.get(points.size() - 2), (Point2D) points.get(points.size() - 1),
				getEndArrowHead(), endLabel, false);
	...
	private void drawString(Graphics2D g2, Point2D p, Point2D q, ArrowHead arrow, String s, boolean center) {
		...
		if (arrow == arrow.LOLLIPOP)
			g2.translate(b.getX() + 50, b.getY() - 10);
		else
	...
	public Rectangle2D getBounds() {
		...
		r.add(getStringBounds((Point2D) points.get(1), (Point2D) points.get(0), getStartArrowHead(), startLabel,
				false));
		r.add(getStringBounds((Point2D) points.get(points.size() / 2 - 1), (Point2D) points.get(points.size() / 2),
				null, middleLabel, true));
		r.add(getStringBounds((Point2D) points.get(points.size() - 2), (Point2D) points.get(points.size() - 1),
				getEndArrowHead(), endLabel, false));
		return r;
	}
	...
	public Shape getShape() {
		...
		path.append(getEndArrowHead().getPath((Point2D) points.get(points.size() - 2),
	...
	public ArrayList<Point2D> getPoints() {
		...
		if (!BentStyle.AUTO.equals(getBentStyle())) {
			...
			Point2D point = bentStylePoints.get(bentStylePoints.size()-1);
		...
		if ((Direction.NORTH.equals(startingCardinalDirection) || Direction.SOUTH.equals(startingCardinalDirection))
				&& (Direction.NORTH.equals(endingCardinalDirection)
						|| Direction.SOUTH.equals(endingCardinalDirection))) {
